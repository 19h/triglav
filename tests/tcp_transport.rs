...
async fn test_tcp_transport_send_recv() {
    ...
    let config = TransportConfig::default();
    let transport = TcpTransport::connect(server_addr, None, &config)
        .await
        .unwrap();
...
async fn test_tcp_transport_with_bind_addr() {
    ...
    let transport = TcpTransport::connect(server_addr, Some(bind_addr), &config)
        .await
        .unwrap();
...
async fn test_tcp_recv_message_too_large() {
    ...
    assert!(
        result.is_err(),
        "Should fail when message is larger than buffer"
    );
}

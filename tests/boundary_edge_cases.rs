...
use triglav::protocol::{Packet, PacketType, PacketFlags, HEADER_SIZE, MAX_PAYLOAD_SIZE};
...
fn test_empty_encryption() {
    ...
    assert!(!ciphertext.is_empty(), "Ciphertext should have auth tag even for empty plaintext");
    assert_eq!(ciphertext.len(), 16, "Empty plaintext should only have auth tag");
...
fn test_payload_corruption_detected() {
    ...
    assert!(decoded.is_ok(), "Payload corruption not detected by header checksum (expected)");
...
fn test_zero_session_id() {
    ...
    assert!(packet.is_ok(), "Zero session ID should be valid (application decides semantics)");
}
...
fn test_session_id_uniqueness() {
    ...
    assert_ne!(s1.as_bytes(), s2.as_bytes(), "Random session IDs should be unique");
}
...
fn test_timestamp_in_packet() {
    ...
    if let Some(d) = delay {
        assert!(d < Duration::from_secs(1), "One-way delay should be small for just-created packet");
    }
}
...
fn test_all_packet_types() {
    ...
    for ptype in types {
        ...
        assert_eq!(decoded.header.packet_type, ptype, "Packet type should be preserved");
    }
}
...
fn test_binary_payload() {
    ...
    assert_eq!(decoded.payload, payload, "Binary payload should be preserved exactly");
}
...
fn test_handshake_with_payload() {
    ...
    assert_eq!(received_payload, custom_payload, "Handshake payload should be delivered");
}

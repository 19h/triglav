                ...
                let (addr, netmask, broadcast) = match family {
                    libc::AF_INET => {
                        ...
                        let broadcast = if !ifa.ifa_dstaddr.is_null()
                            && (ifa.ifa_flags as i32 & libc::IFF_BROADCAST) != 0
                        {
                        ...
                        let broadcast: Option<IpAddr> = {
                            ...
                            if (ifa.ifa_flags as i32 & libc::IFF_BROADCAST) != 0 {
                                ...
                                let union_ptr =
                                    std::ptr::addr_of!(ifa.ifa_ifu) as *const *mut libc::sockaddr;
...
mod tests {
    ...
    fn test_guess_interface_type() {
        assert_eq!(
            guess_interface_type("lo"),
            crate::types::InterfaceType::Loopback
        );
        assert_eq!(
            guess_interface_type("eth0"),
            crate::types::InterfaceType::Ethernet
        );
        assert_eq!(
            guess_interface_type("enp0s3"),
            crate::types::InterfaceType::Ethernet
        );
        assert_eq!(
            guess_interface_type("wlan0"),
            crate::types::InterfaceType::Wifi
        );
        assert_eq!(
            guess_interface_type("wlp2s0"),
            crate::types::InterfaceType::Wifi
        );
        assert_eq!(
            guess_interface_type("tun0"),
            crate::types::InterfaceType::Tunnel
        );
        assert_eq!(
            guess_interface_type("wg0"),
            crate::types::InterfaceType::Tunnel
        );
    }

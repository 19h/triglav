...
use triglav::multipath::{
    Scheduler, SchedulerConfig, SchedulingStrategy,
    Uplink, UplinkConfig, NatType,
};
use triglav::transport::TransportProtocol;
use triglav::types::{UplinkId, ConnectionState, InterfaceType};
...
fn test_adaptive_selects_best_uplink() {
    ...
    assert_eq!(selected[0], uplink_good.numeric_id(), "Should prefer lower latency uplink");
}
...
fn test_adaptive_considers_loss() {
    ...
    assert_eq!(selected[0], uplink_low_loss.numeric_id(), "Should prefer lower loss uplink");
}
...
fn test_adaptive_considers_nat() {
    ...
    assert_eq!(selected[0], uplink_no_nat.numeric_id(), "Should prefer non-NATted uplink");
}
...
fn test_wrr_strategy_creation() {
    ...
    let scheduler = Scheduler::new(config);
    assert_eq!(scheduler.config().strategy, SchedulingStrategy::WeightedRoundRobin);
}
...
fn test_wrr_respects_weights() {
    ...
    assert!(high_count > low_count, 
        "Higher weight uplink should be selected more often: high={}, low={}", high_count, low_count);
}
...
fn test_lowest_latency_strategy() {
    ...
    let uplinks = vec![uplink_slow.clone(), uplink_medium.clone(), uplink_fast.clone()];
    
    // Should always select the fastest
    for _ in 0..10 {
        let selected = scheduler.select(&uplinks, None);
        assert_eq!(selected[0], uplink_fast.numeric_id(), "Should always select lowest latency");
    }
}
...
fn test_lowest_loss_strategy() {
    ...
    let selected = scheduler.select(&uplinks, None);
    assert_eq!(selected[0], uplink_clean.numeric_id(), "Should select uplink with lowest loss");
}
...
fn test_redundant_strategy() {
    ...
    assert_eq!(selected.len(), 3, "Redundant strategy should return all usable uplinks");
...
fn test_primary_backup_strategy() {
    ...
    let uplinks = vec![uplink_cellular.clone(), uplink_wifi.clone(), uplink_ethernet.clone()];
    ...
    assert_eq!(selected[0], uplink_ethernet.numeric_id(), "Ethernet should be primary");
    
    if selected.len() >= 2 {
        // WiFi should be backup
        assert_eq!(selected[1], uplink_wifi.numeric_id(), "WiFi should be backup");
    }
}
...
fn test_primary_backup_failover() {
    ...
    let selected = scheduler.select(&uplinks, None);
    assert_eq!(selected[0], uplink_backup.numeric_id(), "Backup should become primary after failure");
}
...
fn test_ecmp_aware_different_flows() {
    ...
    assert_eq!(seen_uplinks.len(), 2, "Different flows should distribute across uplinks");
}
...
fn test_ecmp_aware_consistency() {
    ...
    for _ in 0..50 {
        let selected = scheduler.select(&uplinks, Some(flow_id));
        assert_eq!(selected, expected, "ECMP should be consistent for same flow");
    }
}
...
fn test_path_stickiness_different_flows() {
    ...
    for _ in 0..10 {
        assert_eq!(scheduler.select(&uplinks, Some(flow_a)), selection_a, "Flow A should be sticky");
        assert_eq!(scheduler.select(&uplinks, Some(flow_b)), selection_b, "Flow B should be sticky");
    }
}
...
fn test_select_all_unusable() {
    ...
    assert!(selected.is_empty(), "Should return empty when all uplinks unusable");
}
...
fn test_needs_probe() {
    ...
    assert!(scheduler.needs_probe(&uplink), "Should need probe initially");
    
    // After recording probe
    scheduler.record_probe(uplink.numeric_id());
    assert!(!scheduler.needs_probe(&uplink), "Should not need probe immediately after recording");
}
...
fn test_scheduler_cleanup() {
    ...
    assert!(scheduler.needs_probe(&uplink), "Should need probe after cleanup");
}
...
fn test_strategy_differences() {
    ...
    assert_eq!(ll_selected[0], uplink_fast_lossy.numeric_id(), "LowestLatency should prefer fast");
    ...
    assert_eq!(loss_selected[0], uplink_slow_clean.numeric_id(), "LowestLoss should prefer clean");
}

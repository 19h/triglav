    ...
    fn emit_diff_events(
        ...
        for name in old.keys() {
            if !new.contains_key(name) {
                ...
                let _ = self.event_tx.send(NetworkEvent::InterfaceRemoved(name.clone()));
            }
        }
        ...
        for (name, new_ifaces) in new {
            if let Some(old_ifaces) = old.get(name) {
                ...
                if old_up != new_up || old_running != new_running {
                    debug!("Interface state changed: {} up={} running={}", name, new_up, new_running);
                ...
                if let Some(iface) = new_ifaces.first() {
                    ...
                    let _ = self.event_tx.send(NetworkEvent::InterfaceAdded(iface.clone()));
                }
            }
        }
    }
    ...
    async fn start_netlink_monitor(&self) {
        ...
        tokio::spawn(async move {
            ...
            let (connection, handle, mut messages) = match rtnetlink::new_connection() {
                Ok(c) => c,
                Err(e) => {
                    error!("Failed to create netlink connection: {}, falling back to polling", e);
                    // Fallback to polling

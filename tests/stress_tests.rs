...
async fn test_high_message_throughput() {
    let server = StressServer::new("127.0.0.1:0".parse().unwrap()).await.unwrap();
    ...
    println!("  Server received: {} packets, {} bytes", srv_recv, srv_bytes_recv);
    println!("  Server sent: {} packets, {} bytes", srv_sent, srv_bytes_sent);
...
async fn test_large_message_volume() {
    ...
    let elapsed = start.elapsed();
    let throughput_mbps = (bytes_sent + bytes_received) as f64 / elapsed.as_secs_f64() / 1_000_000.0;
...
async fn test_many_flows() {
    ...
    for (i, flow_id) in flows.iter().enumerate() {
        ...
        if manager.send_on_flow(Some(*flow_id), msg.as_bytes()).await.is_ok() {
            sent += 1;
        }
    }
    ...
    assert_eq!(manager.active_flow_count(), 0, "All flows should be released");
...
async fn test_scheduler_under_load() {
    ...
    // Should be very fast
    assert!(rate > 100_000.0, "Scheduler should handle >100k selections/s");
}
...
async fn test_memory_not_leaked_on_flow_churn() {
    ...
    assert_eq!(manager.active_flow_count(), 0, "No flows should remain after churn");
...
async fn test_long_running_stability() {
    ...
    assert!(success_rate > 0.95, "Should have >95% success rate, got {:.1}%", success_rate * 100.0);
...
fn test_packet_encode_decode_stress() {
    ...
    assert!(rate > 50_000.0, "Should handle >50k encode/decode per second");
}
...
fn test_encryption_stress() {
    ...
    assert!(rate > 10_000.0, "Should handle >10k encrypt/decrypt per second");
}

...
use triglav::multipath::{MultipathManager, MultipathConfig, MultipathEvent, UplinkConfig};
use triglav::protocol::{Packet, PacketType, PacketFlags, HEADER_SIZE};
use triglav::transport::TransportProtocol;
use triglav::types::{SessionId, SequenceNumber, UplinkId, ConnectionState, UplinkHealth};
...
async fn test_single_uplink_failure() {
    let server = MockServer::new("127.0.0.1:0".parse().unwrap()).await.unwrap();
...
async fn test_all_uplinks_fail_event() {
    ...
    assert!(got_disconnect || manager.uplink_count() == 0, 
...
async fn test_uplink_failover_to_backup() {
    ...
    assert_eq!(binding, Some(backup_id), "Flow should be bound to backup after failover");
...
async fn test_server_stops_responding() {
    ...
    assert!(recv_result.is_ok(), "Should receive echo when server is responding");
    ...
    assert!(send_result.is_ok(), "UDP send completes even without response");
    ...
    assert!(requests_after > requests_before, "Server still receives packets");
...
async fn test_event_broadcasting() {
    ...
    let has_connected = events.iter().any(|e| matches!(e, MultipathEvent::Connected));
    let has_uplink_connected = events.iter().any(|e| matches!(e, MultipathEvent::UplinkConnected(_)));
    
    // At least one of these should be true
    assert!(has_connected || has_uplink_connected || !events.is_empty(), 

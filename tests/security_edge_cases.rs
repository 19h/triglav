...
use triglav::crypto::{KeyPair, SecretKey, PublicKey, NoiseSession};
use triglav::error::{Result, CryptoError};
use triglav::protocol::{Packet, PacketType, PacketFlags, HEADER_SIZE};
use triglav::types::{SessionId, SequenceNumber};
...
fn test_zero_key_rejected() {
    ...
    if let Ok(mut session) = result {
        ...
        assert!(hs.is_ok(), "Handshake write should succeed (key validation is application responsibility)");
    }
}
...
fn test_ciphertext_extension_detected() {
    ...
    if let Ok(decrypted) = result {
        assert_eq!(decrypted, plaintext, "Extra data should not appear in plaintext");
    }
}
...
fn test_empty_ciphertext_rejected() {
    ...
    let (_, mut server) = complete_handshake(
        &client_kp.secret,
        &server_kp.secret,
        &server_kp.public,
    );
...
fn test_short_ciphertext_rejected() {
    ...
    assert!(result.is_err(), "Should reject ciphertext shorter than auth tag");
}
...
fn test_cross_session_decryption_fails() {
    ...
    let (mut client1, mut server1) = complete_handshake(
        &client1_kp.secret,
        &server_kp.secret,
        &server_kp.public,
    );
    
    let (mut client2, mut server2) = complete_handshake(
        &client2_kp.secret,
        &server_kp.secret,
        &server_kp.public,
    );
...
fn test_different_sessions_different_keys() {
    ...
    assert_ne!(ct1, ct2, "Same plaintext should produce different ciphertexts in different sessions");
    
    // Cross-decryption should fail
    assert!(server2.decrypt(&ct1).is_err(), "Cross-session decryption should fail");
    assert!(server1.decrypt(&ct2).is_err(), "Cross-session decryption should fail");
}
...
fn test_packet_checksum_validation() {
    ...
    assert!(result.is_err(), "Should detect corrupted header via checksum");
}
...
fn test_packet_version_mismatch() {
    ...
    if let Ok(p) = result {
        ...
        assert_ne!(p.header.version, triglav::PROTOCOL_VERSION,
...
fn test_encrypt_before_handshake_fails() {
    ...
    assert!(result.is_err(), "Should not encrypt before handshake complete");
}
...
fn test_decrypt_before_handshake_fails() {
    ...
    assert!(result.is_err(), "Should not decrypt before handshake complete");
}
...
fn test_handshake_after_transport_fails() {
    ...
    assert!(result.is_err(), "Should not allow handshake after transport mode");
}
...
fn test_verify_with_wrong_public_key() {
    ...
    let result = SigningKeyPair::verify_with_public(
...
fn test_secure_compare_unequal() {
    ...
    assert!(!secure_compare(&a, &b), "Different arrays should not compare equal");
}
...
fn test_secure_compare_different_lengths() {
    ...
    assert!(!secure_compare(&a, &b), "Different length arrays should not compare equal");
}

...
fn bind_to_interface(socket: &Socket, config: &SocketConfig) -> Result<()> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if ret != 0 {
            ...
            debug!(
                "Bound socket to interface {} via SO_BINDTODEVICE",
                interface
            );
        }
    }
    ...
    {
        // Get interface index
        let idx = crate::util::if_nametoindex(interface).ok_or_else(|| {
            TransportError::SocketError(format!("Interface not found: {}", interface))
        })?;
...
fn set_socket_mark(socket: &Socket, mark: u32) -> Result<()> {
    ...
    if ret != 0 {
        ...
        return Err(TransportError::SocketError(format!("SO_MARK failed: {}", err)).into());
    }
...
pub fn bind_socket(is_ipv6: bool, is_udp: bool, config: &SocketConfig) -> Result<Socket> {
    let addr = if is_ipv6 {
